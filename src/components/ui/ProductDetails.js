...
export const ProductActions = (
  ...
  const customDownloadBtn = downloadUrl
    ? `<a href="${downloadUrl}" download class="flex-1 px-8 py-4 bg-secondary text-white font-heading font-semibold rounded-lg hover:bg-secondary/90 shadow-lg transition-all flex justify-center items-center gap-2">
  ...
  const pdfButton =
    !showPdf || customDownloadBtn
      ...
      : pdfUrl
        ...
        : `<button disabled class="px-8 py-4 bg-gray-300 border border-gray-400 text-gray-600 font-heading font-semibold rounded-lg cursor-not-allowed opacity-60" aria-label="Button">
        Download PDF
      </button>`;

  const quoteBtn = !hideQuote
    ? `<button onclick="window.openQuoteModal('${escapedName}', '${productId}')" class="flex-1 px-8 py-4 bg-primary text-white font-heading font-semibold rounded-lg hover:bg-primary-hover shadow-lg shadow-primary/30 transition-all cursor-pointer" aria-label="Open Quote Modal">
        Request Quote
      </button>`
    : "";
