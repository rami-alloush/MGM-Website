...
export const CategoryCard = ({ key, title, description, icon, imageSrc }) => {
  return `
    <a href="#/products/${key}" class="group bg-white border border-silver rounded-2xl overflow-hidden hover:border-primary hover:shadow-xl transition-all flex flex-col h-full">
      <div class="w-full bg-clinical-gray overflow-hidden flex-shrink-0 relative">
        <img src="${imageSrc}" alt="${title}" class="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform" onerror="this.parentElement.innerHTML='<div class=\\'flex items-center justify-center h-full text-primary\\'><div class=\\'w-24 h-24\\'>${icon}</div></div>'" />
        <button class="absolute bottom-3 right-3 w-10 h-10 bg-white/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-white" onclick="event.preventDefault(); event.stopPropagation(); window.openLightbox('${imageSrc}', '${title}')" aria-label="View image in fullscreen">
